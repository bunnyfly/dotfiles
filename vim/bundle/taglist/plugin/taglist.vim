...
function! s:Tlist_Window_Create()
	...
	""""" NERDTREE WINDOW SHARE HACK
    " Create the taglist window
    "exe 'silent! ' . win_dir . ' ' . win_size . 'split ' . wcmd
		if !exists('t:NERDTreeBufName') || bufwinnr(t:NERDTreeBufName) == -1
			exe 'silent! ' . win_dir . ' ' . win_size . 'split ' . wcmd
		else
			let winnumNERD = bufwinnr(t:NERDTreeBufName)
			if winnumNERD != -1
				" Jump to the existing NERDTree window
				if winnr() != winnumNERD
					exe winnumNERD . 'wincmd w'
				endif
			endif
			exe 'silent! rightbelow split ' . wcmd
		endif
